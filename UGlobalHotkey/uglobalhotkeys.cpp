...
UGlobalHotkeys::UGlobalHotkeys(QWidget* parent) : QWidget(parent) {
#if defined(Q_OS_LINUX)
  qApp->installNativeEventFilter(this);
  QWindow wndw;
  void* v = qApp->platformNativeInterface()->nativeResourceForWindow(
      "connection", &wndw);
...
#if defined(Q_OS_MAC)
OSStatus macHotkeyHandler(EventHandlerCallRef nextHandler,
                          EventRef theEvent,
                          void* userData) {
...
#ifdef Q_OS_WIN
  foreach (size_t id, Registered) { this->unregisterHotkey(id); }
#elif defined(Q_OS_LINUX)
  foreach (size_t id, Registered.keys())
...
#if defined(Q_OS_WIN)
  for (QSet<size_t>::iterator i = Registered.begin(); i != Registered.end();
       i++) {
...
void UGlobalHotkeys::regLinuxHotkey(const UKeySequence& keySeq, size_t id) {
  ...
  xcb_grab_key(X11Connection, 1, X11Wid, data.mods, data.keyCode,
               XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
  // NumLk
  xcb_grab_key(X11Connection, 1, X11Wid, data.mods | XCB_MOD_MASK_2,
               data.keyCode, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
...
void UGlobalHotkeys::unregLinuxHotkey(size_t id) {
  ...
  xcb_ungrab_key(X11Connection, data.keyCode, X11Wid,
